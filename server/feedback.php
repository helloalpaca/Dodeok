<?php
  ...
  if($userid != "" and $classname != ""){
    ...
    while($info=mysqli_fetch_array($sql_result)){
      ...
      //echo $emotiontime;
      if($emotiontime != null){
      $sql2 = "select * from feedback where feedtime = '$emotiontime' && id = '$userid' && feedid = '$id'";
      $sql_result2 = mysqli_query($connect,$sql2);
      $info2 = mysqli_fetch_array($sql_result2);

      array_push($data, array('id'=>$info['id'],'emotion'=>$info['emotion'],'emotiontime'=>$info['emotiontime'],'feedback'=>$info2['feedback'],'feedtime'=>$info2['feedtime']));
}
    }
  }
